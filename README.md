...
- 支持监控捕鱼装备耐久，在安全钓鱼空档调用 AutoDuty 的修理流程，完成后恢复钓鱼目标。
...
也可以下载 [FisherDutyScheduler 0.7.1](release/FisherDutyScheduler-0.7.1.zip)，解压后将 `FisherDutyScheduler.dll` 添加为开发插件。
...
- **自动维护捕鱼装备 / 修理阈值**：最低耐久达到阈值后，在安全空档调用 AutoDuty 当前配置的自修或 NPC 修理流程。

NPC 修理需要足够金币；自修需要对应等级的生产职业和暗物质。使用自修时，实际触发阈值取调度器与 AutoDuty 两边设置中的较低值。
